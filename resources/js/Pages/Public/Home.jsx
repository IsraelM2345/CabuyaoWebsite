...
/**
 * AutoCarousel component - handles automatic scrolling for carousel sliders
 * Always auto-scrolls, pauses temporarily on user interaction then resumes
 */
function AutoCarousel({ sliderId, interval = 3000 }) {
    useEffect(() => {
        ...
        // Start auto-scroll
        scrollInterval = setInterval(scrollRight, interval);

        // Pause temporarily on user interaction, then resume
        ...
        slider.addEventListener("mouseenter", pauseAndResume);
        slider.addEventListener("mouseleave", resumeOnLeave);
        slider.addEventListener("touchstart", pauseAndResume, {
            passive: true,
        });
        slider.addEventListener("touchend", resumeOnLeave);

        return () => {
            clearInterval(scrollInterval);
            if (resumeTimeout) clearTimeout(resumeTimeout);
            slider.removeEventListener("mouseenter", pauseAndResume);
            slider.removeEventListener("mouseleave", resumeOnLeave);
            slider.removeEventListener("touchstart", pauseAndResume);
            slider.removeEventListener("touchend", resumeOnLeave);
        };
    }, [sliderId, interval]);
...
export default function Home() {
    ...
    const [councilorAutoPlay, setCouncilorAutoPlay] = useState(true);
        ...
        <div className="min-h-screen bg-white font-sans text-gray-800">
            ...
            <div className="py-24 px-6 max-w-6xl mx-auto">
                ...
                {viceMayor ? (
                    <a
                        href="/council"
                        className="flex flex-col items-center bg-white rounded-2xl shadow-xl border border-gray-100 px-6 pb-12 pt-8 max-w-3xl mx-auto text-center w-full flex-shrink-0 group/card hover:shadow-2xl transition-all duration-300 mb-24"
                    >
                ...
                <div className="relative max-w-7xl mx-auto px-12 md:px-16">
                    ...
                    <div
                        ...
                        onScroll={(e) => {
                            ...
                            if (prevBtn && slider.scrollLeft <= 10) {
                        ...
                        onMouseEnter={() => setCouncilorAutoPlay(false)}
                        onMouseLeave={() => setCouncilorAutoPlay(true)}
                        onTouchStart={() => setCouncilorAutoPlay(false)}
                    >
                        <style
            ...
            {/* Auto-play carousel logic for councilors */}
            {councilors.length > 4 && (
                <AutoCarousel sliderId="councilor-slider" interval={3000} />
            )}
